...
import type { DesignEngine } from '@buildev/pen-engine';
import { DEFAULT_PAGE_ID } from '@/stores/document-tree-utils';
import { appStorage } from '@/utils/app-storage';

const PREFS_KEY = 'buildev-canvas-preferences';

export type RightPanelTab = 'design' | 'code';

export type IdeActivity = 'explorer' | 'code';

export type IdeDiagnostic = { id: string; message: string; severity: 'error' | 'warning' };
...
interface CanvasStoreState {
  ...
  designMdPanelOpen: boolean;
  codePanelOpen: boolean;
  /** Full-window IDE-style shell (separate from right panel Code tab). Not persisted. */
  ideModeOpen: boolean;
  ideActivity: IdeActivity;
  rightPanelTab: RightPanelTab;
  figmaImportDialogOpen: boolean;
  pendingFigmaFile: File | null;
  exportDialogOpen: boolean;
  /** Share dialog (editor URL, MCP, Git, path) */
  shareDialogOpen: boolean;
  activePageId: string | null;
  /** IDE bottom panel "Problems" (channel A parse, etc.) */
  ...
  setIdeModeOpen: (open: boolean) => void;
  setIdeActivity: (activity: IdeActivity) => void;
  ...
  setIdeDiagnostics: (items: IdeDiagnostic[]) => void;
  clearIdeDiagnostics: () => void;
...
export const useCanvasStore = create<CanvasStoreState>((set, get) => ({
  ...
  setIdeModeOpen: (open) =>
    set({
      ideModeOpen: open,
      ...(!open ? { ideActivity: 'explorer' as IdeActivity, ideDiagnostics: [] } : {}),
    }),
  setIdeDiagnostics: (items) => set({ ideDiagnostics: items }),
  clearIdeDiagnostics: () => set({ ideDiagnostics: [] }),
  setIdeActivity: (activity) => set({ ideActivity: activity }),
