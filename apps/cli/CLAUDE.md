# apps/cli/ — Buildev CLI

The `op` command-line tool controls the Buildev desktop app or web server from the terminal.
...
│       ├── install.ts    install, uninstall (buildev-skill for AI agents)
...
├── dist/                 Compiled output (buildev-cli.cjs)
├── package.json          @bryfar/buildev, bin: { op }
└── README.md
```

## Commands

- **Compile:** `bun run cli:compile` (esbuild to `dist/buildev-cli.cjs`)
...
- **Skill bundle:** `bun run cli:bundle-skill` pre-generates `skill-bundle.json` from `../buildev-skill/`, embedded into the binary by esbuild. Falls back to git clone at runtime if the bundle is empty.
